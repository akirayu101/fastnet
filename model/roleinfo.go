...
func NewRoleInfo() *RoleInfo {
	return &RoleInfo{Role_Id: 1, Role_Name: "summer", Role_Level: 1, Role_Attr: 1, MaxSkills: 3}
}
...
func (r *RoleInfo) GetRoleInfo(uid uint64, role_id uint8) (err error) {
	err = db.QueryRow("Select * FROM user_role WHERE uid = ? AND role_id = ?", uid, role_id).Scan(&r.Id, &r.Uid, &r.Role_Id, &r.Role_Name, &r.Role_Level, &r.Role_Attr, &r.MaxSkills)
...
func (r RoleInfo) GetRoleList(uid uint64) (urList []RoleInfo, err error) {
	...
	for rows.Next() {
		ur := new(RoleInfo)
		err = rows.Scan(&ur.Id, &ur.Uid, &ur.Role_Id, &ur.Role_Name, &ur.Role_Level, &ur.Role_Attr, &ur.MaxSkills)
		if err != nil {
			log.Println(err)
			return
		}
