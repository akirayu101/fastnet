...
func (u *UserInfo) GetUserInfo(uid uint64) (err error) {
	err = db.QueryRow("SELECT uid, username, role_id, cash, diamond, maxskills FROM users WHERE uid = ?", uid).Scan(&u.Uid, &u.Username, &u.Role_Id, &u.Cash, &u.Diamond, &u.MaxSkills)
...
func (u *UserInfo) AddMaxSkills(uid uint64, diamond int32) (err error) {
	_, err = db.Query("UPDATE users SET maxskills = maxskills + 1 WHERE uid = ? ", uid)
	if err != nil {
		log.Println(err)
	}

	u.MaxSkills = u.MaxSkills + 1
	return
}
